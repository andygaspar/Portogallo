...
class AttentionNet(nn.Module):
    ...
    def forward(self, state, actions, num_flights, num_airlines):
    ...
    def pick_action(self, state, action, num_flights, num_airlines):
        with torch.no_grad():
            scores = self.forward(state.to(self.device), action.to(self.device),
    ...
    def update_weights_episode(self, batch: tuple, gamma: float = 1.0):
        ...
        states, actions, rewards = (element.to(self.device) for element in batch[:-2])
        ...
        self.zero_grad()
        Q = self.forward(states, actions_tensor, num_flights, num_airlines)
